...
All function arguments and return values passed between workflow and step functions must be serializable. Workflow DevKit uses a custom serialization system built on top of [devalue](https://github.com/Rich-Harris/devalue). This system supports standard JSON types, as well as a few additional popular Web API types.
...
- All data passed between workflow and step functions must be serializable
- Standard JSON types and many Web API types are supported
- Streams can be passed through workflows but must be consumed in step functions
- Request/Response body methods are automatically treated as steps when called in workflows
- The serialization system enables durable execution across suspensions and resumptions

## Reference

- [Workflow DevKit Serialization Documentation](https://useworkflow.dev/docs/foundations/serialization)
